...
public class Dungeon {
    ...
    private static DungeonCharacter[] chooseHero() {
        System.out.print("Please enter how many characters on your team you would like to play with(3 max)\n");
        int n = readInt();
        while(n > 3 || n <= 0)
        {
    ...
    private static int select(DungeonCharacter[] choice){
        ...
        System.out.println();
        int num = readInt();
        while(num > choice.length || num < 1)
        {
            System.out.println("Invalid selection, try again");
            num = readInt();
        }
        num--;
        if(!choice[num].isAlive())
